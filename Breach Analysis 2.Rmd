...
The data are made available as a google doc. I've been unable (so far) to write a successful R program to download the data directly, so in the interest of time I just downloaded their raw data as a .csv.  
...
You'll note that one row has blank data for number or recrods stolen. I'm not sure why that is. I plan to look into it later but don't have time right now. Judging from a back-of-the-envelope calculation I don't think it biases the conclusions of this analysis.  
...
Breachlm <- lm(Data_Sensitivity~Year, data = AggBreaches)
Hacklm <- lm(Data_Sensitivity~Year, data = AggHacks)

#summary(BreachSenslm)

BreachPerYear<-Breachlm$coefficient[2]
HackPerYear<-Hacklm$coefficient[2]

HBratio = as.integer(100.*HackPerYear/BreachPerYear)


```


As can be seen from the data, breaches from hacks also vary significantly from year to year, but have resulted in a generally upward trend of number of records being lost. The trend line apparently rises faster than the trend line for the overall breaches. Indeed hacks per year `r HackPerYear` are about `r HBratio` % higher than breaches per year `r BreachPerYear`.   
...
BearchSensPerYear<-BreachSenslm$coefficient[2]
HackSensPerYear<-HackSenslm$coefficient[2]
